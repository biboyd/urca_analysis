...
def main(fname):
    ...
    s = yt.SlicePlot(ds, ortho_to_dipole, 'velocity_magnitude', width=(1.2e3, 'km'), 
                     center=ds.domain_center, data_source=conv_sph, north_vector=dipole)
    
    s.set_unit('velocity_magnitude', 'km/s')
    s.set_zlim('all', 0., 60.)
    s.set_log('all', False)
    s.set_cmap('all', "cividis")
    s.save("vel_magnitude_dipole/")

...
def main(fname, field, cmap='viridis'):
    # load dataset
    ds = yt.load(fname)
    ...
    s = yt.SlicePlot(ds, ortho_to_dipole, field, width=(1.2e3, 'km'), data_source=conv_sph, north_vector=dipole)
    s.set_log('all', False)
    s.set_cmap('all', cmap)
    s.save()
...
if __name__ == "__main__":
    if len(argv) == 4:
        main(argv[1], argv[2], cmap=argv[3])
    else:
        main(argv[1], argv[2])

...
for j, file in enumerate(all_files):
    ...
    if sim_time < ds.quan(200, 's'):
        ...
        if timestep in old_conv_time[:, 2] and timestep in old_conv_zone[:, 2] and timestep in old_magvel[:, 2]:
            ...
            inflow_conv_reg = sph.cut_region(["obj[('boxlib', 'radial_velocity')] < 0."])
            outflow_conv_reg = sph.cut_region(["obj[('boxlib', 'radial_velocity')] > 0."])

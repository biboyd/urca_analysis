...
parser.add_argument('-val', '--vort_val', type=float, default=10, help='vorticity for transfer function. (Default is 1e-4).')
parser.add_argument('-sig', '--vort_sigma', type=float, default=0.05, help='vorticity transfer function width parameter. (Default is 0.01).')
...
# Create Sources
so_vort = create_volume_source(core, field)
so_urca_shell = create_volume_source(core, 'rho')

# Assign Transfer Functions to Sources
# give buffer on bounds
bounds = np.array([args.vort_val/2, 2*args.vort_val])

tfh = TransferFunctionHelper(ds)
tfh.set_field(field)
tfh.set_log(True)
tfh.grey_opacity = args.grey
tfh.set_bounds((0.001, 15))
tfh.build_transfer_function()

#tfh.tf.add_gaussian(np.log10(9.5e-4), args.frac_sigma**2, plasma(1., args.alpha))
tfh.tf.add_gaussian(np.log10(8), 1.5*args.vort_sigma**2, plasma(1., args.alpha))
tfh.tf.add_gaussian(np.log10(5), args.vort_sigma**2, plasma(.5, args.alpha))
tfh.tf.add_gaussian(np.log10(4), args.vort_sigma**2, plasma(.25, args.alpha/2.))
#tfh.tf.add_gaussian(np.log10(4), args.vort_sigma**2, plasma(.25, args.alpha))
tfh.tf.add_gaussian(np.log10(1), args.vort_sigma**2, plasma(0.0, args.alpha/20))
...
tfh.plot(f"{args.outprefix}{ds.basename}_tfun_vort.png")#, profile_field=('boxlib', 'urca_ratio'))
so_vort.transfer_function = tfh.tf
...
sc.render()
sc.save(f"{args.outprefix}{ds.basename}_rendering_vorticity.png", sigma_clip=4, render=False)
sc.save_annotated(f"{args.outprefix}{ds.basename}_annotated_rendering_vorticity.png", sigma_clip=4,  render=False)

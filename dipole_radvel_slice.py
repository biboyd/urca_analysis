...
def main(fname):
    ...
    s = yt.SlicePlot(ds, ortho_to_dipole, 'radial_velocity', width=(1.2e3, 'km'), center=ds.domain_center, data_source=conv_sph, north_vector=dipole)
    
    s.set_unit('radial_velocity', 'km/s')
    s.set_zlim('all', -2e2, 2e2)
    s.set_log('all', linthresh=1e-1)
    s.set_cmap('all', "RdBu_r")
    s.save()
